...
function getDaysAgo(timestamp) {
  ...
  // 3. 用 .diff() 計算天數差異
  const dayDiff = dayjs().diff(dayjs.unix(timestamp), 'day');
  const result = dayDiff === 0 ? "今天" : `${dayDiff}天前`;
  return result;
}
...
function isOrderOverdue(timestamp) {
  const dayDiff = dayjs().diff(dayjs.unix(timestamp), 'day');
  const result = dayDiff >= 7 ? true : false;
  return result;
}
...
function validateOrderUser(data) {
  // 請實作此函式
  const isNameValid = data.name !== "" ? true : false 
  const telRegex = /^09\d{8}$/;
  const isTelValid = telRegex.test(data.tel);

  const isEmailValid = data.email.includes('@');
  const isAddressValid = data.address !== "" ? true : false;
  const validPayments = ['ATM', 'Credit Card', 'Apple Pay'];
  const isPaymentValid = validPayments.includes(data.payment);

  if (isNameValid && isTelValid && isEmailValid && isAddressValid && isPaymentValid) {
    return {
      isValid: true,
      errors: [],
    }
    ...
    if (!isNameValid) errors.push(`名字不可為空`);
    if (!isTelValid) errors.push(`手機號碼格式有錯誤`);
    if (!isEmailValid) errors.push(`e-mail格式有錯誤`);
    if (!isAddressValid) errors.push(`地址格式有錯誤`);
    if (!isPaymentValid) errors.push(`付款方式有錯誤`);
    return {
      isValid: false,
      errors: errors,
    }
...
function generateCartItemId() {
  ...
  const timePart = Date.now().toString(36);
  const randomPart = Math.random().toString(36).slice(2, 10);
  return `CART-${timePart}-${randomPart}`;
}
...
1. axios可以自動解析JSON，fetch需要手動呼叫response.json()

2. axios 會將 HTTP 狀態碼 4xx/5xx 視為錯誤；fetch 只有在網路錯誤時才會reject

3. axios 會自動設定 Content-Type 並將物件轉為 JSON；fetch需要手動設定 headers、JSON.stringfy()
*/
